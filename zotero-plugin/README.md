...
On Windows the plugin directly starts the configured Obsidian executable, bypassing Zotero's external-protocol confirmation dialog; other platforms use the registered protocol handler. Canvas requests deliberately leave the file parameter to the Excalidraw plugin so `.excalidraw.md` files never race with Obsidian's Markdown opener. Literature-card requests send the selected item's key, title, authors, year, optional citation key, and DOI through the same local service at `127.0.0.1:27119`, then open the returned Markdown path. Existing annotation synchronization and canvas mapping behavior are unchanged.
